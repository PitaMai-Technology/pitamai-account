...
watchEffect(() => {
    if ((state.organizationId === undefined || state.organizationId === null) && activeOrganization.value?.data) {
...
async function fetchMembers(query: Partial<Schema>) {
    ...
    try {
        ...
        if (error) {
            toast.add({ title: 'エラー', description: `ユーザー一覧の取得に失敗しました: ${error.message}`, color: 'error' });
            members.value = [];
        ...
        if (data?.members && Array.isArray(data.members)) {
            ...
            total.value = typeof data.total === 'number' ? data.total : data.members.length;
        ...
        toast.add({ title: '成功', description: 'ユーザー一覧を取得しました。', color: 'success' });
    } catch (e: unknown) {
        if (e instanceof Error) {
            toast.add({ title: 'エラー', description: `ユーザー一覧取得中にエラーが発生しました: ${e.message}`, color: 'error' });
        } else {
            toast.add({ title: 'エラー', description: 'ユーザー一覧取得中に予期しないエラーが発生しました', color: 'error' });
        }
        members.value = [];
        total.value = undefined;
    } finally {
...
<template>
    <div class="space-y-6 p-4">
        ...
        <form @submit.prevent="onSubmit($event)" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="block text-sm">Organization ID</label>
                <input v-model="state.organizationId" type="text" class="w-full mt-1 input"
                    placeholder="organization id" />
            </div>

            <div>
                <label class="block text-sm">Limit</label>
                <input v-model.number="state.limit" type="number" min="1" max="100" class="w-full mt-1 input" />
            </div>

            <div>
                <label class="block text-sm">Offset</label>
                <input v-model.number="state.offset" type="number" min="0" class="w-full mt-1 input" />
            </div>
            ...
            <div>
                ...
                <select v-model="state.sortDirection" class="w-full mt-1 input">
                    <option v-for="d in sortDirectionOptions" :key="d.value" :value="d.value">{{ d.label }}</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input v-model="state.filterValue" type="text" class="w-full mt-1 input" placeholder="値（複数はカンマ区切り）" />
            </div>
            ...
            <div class="flex items-end gap-2 md:col-span-3">
                ...
                <button type="button" class="btn" @click="resetForm" :disabled="loading">リセット</button>
            </div>
        </form>
